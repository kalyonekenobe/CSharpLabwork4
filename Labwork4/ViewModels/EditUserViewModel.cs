...
{
	internal class EditUserViewModel : INotifyPropertyChanged, INavigatable<MainNavigationTypes>
	{
		...
		public RelayCommand<object> EditCommand
		{
			get
			{
				return _editUserCommand ??= new RelayCommand<object>(_ => Edit(), CanExecute);
			}
		}
		...
		{
			get => MainNavigationTypes.EditUser;
		}

		public EditUserViewModel(Person person, Action gotoUserList)
		{
			_isEnabled = true;
			_personCandidate = new PersonCandidate();
			_personCandidate.FirstName = person.FirstName;
			_personCandidate.LastName = person.LastName;
			_personCandidate.Email = person.Email;
			_personCandidate.DateOfBirth = person.DateOfBirth;
		...
		private async void Edit()
		{
			...
			try
			{
				...
				if (Email != _editedPerson.Email && await UserExsists(Email))
				{
		...
		private async Task UpdateUser(Person person)
		{
			await _fileRepository.RemoveAsync(_editedPerson);
